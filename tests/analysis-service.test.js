...
test("analysis service retries missing AI cards once and recovers them when the retry succeeds", async () => {
  let batchIndex = 0;
  const service = createAnalysisService({
    cache: new Map(),
    env: {
...
test("analysis service keeps successful AI batches when the retry still fails", async () => {
  let batchIndex = 0;
  const service = createAnalysisService({
    cache: new Map(),
    env: {
